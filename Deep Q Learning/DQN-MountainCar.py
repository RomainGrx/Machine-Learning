...
class Agent():
    ...
    def train(self, state, action, next_state, reward, done, batch_size = 32):
        ...
        if done: self.epsilon = max(0.1, .1 + .9*np.exp(-episode/(EPOCHS//4)))
...
for episode in range(EPOCHS):
    ...
    while not done:
        ...
        loss = agent.train(state, action, next_state, reward, done, batch_size=32)
    ...
    print(f'episode {episode}/{EPOCHS} : reward {np.sum(reward_list)} : loss {np.mean(loss_list):.3f} : epsilon {agent.epsilon:.3f} : reach {REACH}')

agent.network.model.save(f'save_models/{NAME}.h5')
